...
\subsubsection{fitaci函数详解}\label{fitaci}
...
CO\textsubscript{2}
响应曲线数据}{使用 plantecophys 拟合 LI-6400XT CO2 响应曲线数据}}\label{-plantecophys--li-6400xt-co2-}
...
J\textsubscript{max}}{使用 onepoint 单独计算 Vcmax 和 Jmax}}\label{-onepoint--vcmax--jmax}
...
函数}{PhotosynEB 函数}}\label{photosyneb-}
...
的参数}{PhotosynTuzet 的参数}}\label{photosyntuzet-}
...
\chapter{LI-6800 的数据分析}\label{li-6800-}
...
\NormalTok{x <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(A }\OperatorTok{~}\StringTok{ }\NormalTok{Ci, }\DataTypeTok{data =}\NormalTok{ cemp)}
\KeywordTok{summary}\NormalTok{(x)}
\CommentTok{# 利用拟合的结果，提取校准用的斜率和截距}
\NormalTok{x}\OperatorTok{$}\NormalTok{coefficients}

\NormalTok{k <-}\StringTok{ }\NormalTok{x}\OperatorTok{$}\NormalTok{coefficients[}\DecValTok{2}\NormalTok{]}
\NormalTok{z <-}\StringTok{ }\NormalTok{x}\OperatorTok{$}\NormalTok{coefficients[}\DecValTok{1}\NormalTok{]}

\CommentTok{# 校准带叶片测量的数据}
\NormalTok{uleafc <-}\StringTok{ }\NormalTok{uleaf500}
\NormalTok{uleafc}\OperatorTok{$}\NormalTok{A <-}\StringTok{ }\NormalTok{uleafc}\OperatorTok{$}\NormalTok{A }\OperatorTok{-}\StringTok{ }\NormalTok{(k }\OperatorTok{*}\StringTok{ }\NormalTok{uleafc}\OperatorTok{$}\NormalTok{CO2_r }\OperatorTok{+}\StringTok{ }\NormalTok{z)}
...
\KeywordTok{locator}\NormalTok{()}
\NormalTok{cleaf <-}\StringTok{ }\NormalTok{uleafc[}\KeywordTok{which}\NormalTok{(uleafc}\OperatorTok{$}\NormalTok{CO2_r }\OperatorTok{>}\StringTok{ }\FloatTok{15.7} \OperatorTok{&}\StringTok{ }
\StringTok{                        }\NormalTok{uleafc}\OperatorTok{$}\NormalTok{CO2_r }\OperatorTok{<}\StringTok{ }\FloatTok{474.15}\NormalTok{),]}
...
\CommentTok{# 查看拟合数据}
\KeywordTok{summary}\NormalTok{(acifit)}
\KeywordTok{summary}\NormalTok{(acifit_normal)}
...
\NormalTok{alldata <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{A =} 
      \KeywordTok{c}\NormalTok{(acin}\OperatorTok{$}\NormalTok{A, ddata}\OperatorTok{$}\NormalTok{Ac, ddata}\OperatorTok{$}\NormalTok{Aj, cleaf}\OperatorTok{$}\NormalTok{A), }
...
函数}{jip\_plot 及 jip\_dcplot 函数}}\label{jip_plot--jip_dcplot-}
