...
\begin{quote}
该部分内容不建议继续使用，尤其是读取原始数据文件部分。目前代码报错，可能是某个依赖软件包升级导致，但我没时间处理。对于 excel 文件格式的处理，我目前找到了一个自认为比较好的批量转 csv 格式的方案，但是需要 \texttt{python} 的 \texttt{xlwings} 支持，该库比较成熟，不存在依赖地狱的问题，而且我也打包了一个图形界面，批量处理为 csv 后用常规方法导入就行，如果我有时间，会将该部分内容彻底修改。\texttt{python} 方案见链接：\href{https://github.com/zhujiedong/LI6800_excel2csv}{github} 或微信 \href{https://mp.weixin.qq.com/s?__biz=MzU4ODI3NjkzMg==\&mid=2247485489\&idx=2\&sn=9aed981d7624c8e289cb9173c269373a\&chksm=fdde7d9acaa9f48c3dc0d09324d0461b18b4edd4ff56207de54b5152cbfe4a97fdc482b31b82\&token=101697346\&lang=zh_CN\#rd}{微信版}
\end{quote}
...
\href{https://mp.weixin.qq.com/s?__biz=MzU4ODI3NjkzMg==\&mid=2247485449\&idx=1\&sn=914778c6bc1e288c7cb093da403d920e\&chksm=fdde7da2caa9f4b449de6bd4ead60913444c6a6b429e59d6ad80ab5bea10c74e1e0c5da1105d\&token=1056052514\&lang=zh_CN\#rd}{图形界面整理数据}
...
\textbf{当前版本有问题，可能时依赖的 \texttt{stringi} 升级导致，现在没有时间处理。}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(df2[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{15}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{8}\NormalTok{], }\AttributeTok{booktabs =} \ConstantTok{TRUE}\NormalTok{,}
\AttributeTok{caption =} \StringTok{\textquotesingle{}LI{-}6800 regex 方式批量整合数据\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
##  [1] 1.229600 1.768842 1.401876 1.417621 1.385116 1.439096 1.354177 1.467500
##  [9] 1.606849 1.701497 1.406623 1.415589 1.301780 1.281671 1.435706 1.996638
## [17] 1.361774 1.289560 1.401353 1.553608 1.532133 1.662846 1.684584 1.464212
## [25] 1.510672 1.214832 1.381348 1.472921 1.528652 1.561157 1.577983 1.219234
\end{verbatim}
...
\DocumentationTok{\#\#  [1] 24.7381184 18.1379358 10.8055345  3.0239340 {-}0.9144044 26.9519572}
\DocumentationTok{\#\#  [7] 27.5088717 40.9101889 50.1393342 55.3865984 58.0662751 59.3556428}
\end{Highlighting}
\end{Shaded}
...
\DocumentationTok{\#\#  [1] 0.5 0.5 0.5 0.5 0.5 0.5 0.5 0.5 0.5 0.5 0.5 0.5}
\end{Highlighting}
\end{Shaded}
...
\DocumentationTok{\#\#  [1] 0.5404604 0.4946930 0.5055978 0.5131122 0.5091636 0.5010179 0.5118623}
\DocumentationTok{\#\#  [8] 0.4928284 0.4838221 0.5126210 0.4926683 0.4925639}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# 饱和点计算}
\NormalTok{Isat }\OtherTok{\textless{}{-}}\NormalTok{  (}\FunctionTok{sqrt}\NormalTok{((fitlrc\_mrec}\SpecialCharTok{$}\NormalTok{coef[}\DecValTok{2}\NormalTok{,}\DecValTok{1}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ fitlrc\_mrec}\SpecialCharTok{$}\NormalTok{coef[}\DecValTok{3}\NormalTok{,}\DecValTok{1}\NormalTok{])}\SpecialCharTok{/}
...
\sout{可能软件包作者没有更改叶面积的需求，他的软件包只支持原始数据的处理，但这对有更改需求的客户来讲没那么友好，而且这样导致了大量代码的重复，我根据作者原来的代码，结合我自己数据分析的习惯，重新制作了一份软件包，下面代码实现是基于我的软件包实现的，如有需求，也可使用作者原来的代码实现}
...
\hypertarget{high-to-low}{%
\subsubsection{\texorpdfstring{从高到低的 \(CO_2R\) 变化}{从高到低的 CO\_2R 变化}}\label{high-to-low}}
...
\caption{\label{tab:unnamed-chunk-69}jiptest 批量导入数据后的样式}
\centering
...
\caption{\label{tab:unnamed-chunk-70}jiptest DC 数据批量导入数据后的样式}
\centering
\begin{tabular}[t]{rrrl}
\toprule
SECS & FLUOR & NORM\_FLUOR & SOURCE\\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cls }\OtherTok{\textless{}{-}} \FunctionTok{palette.colors}\NormalTok{(}\AttributeTok{n =} \DecValTok{5}\NormalTok{,  }\StringTok{"set 2"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.8}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(jip\_data,}
     \AttributeTok{ylab =} \StringTok{\textquotesingle{}Normalized fluorescence signals\textquotesingle{}}\NormalTok{,}
     \AttributeTok{add\_leg =} \ConstantTok{FALSE}\NormalTok{,}
     \AttributeTok{def\_pch =} \DecValTok{14}\SpecialCharTok{:}\DecValTok{18}\NormalTok{,}
     \AttributeTok{col =}\NormalTok{ cls,}
     \AttributeTok{main =} \StringTok{"Demodulated signals"}\NormalTok{, }\AttributeTok{normalized =} \ConstantTok{FALSE}\NormalTok{)}

\FunctionTok{legend}\NormalTok{(}
  \StringTok{"topleft"}\NormalTok{,}
  \FunctionTok{unique}\NormalTok{(jip\_data}\SpecialCharTok{$}\NormalTok{SOURCE),}
  \AttributeTok{col =}\NormalTok{ cls,}
  \AttributeTok{pch =} \DecValTok{14}\SpecialCharTok{:}\DecValTok{18}\NormalTok{,}
...
\begin{figure}
\centering
\includegraphics{bookdown_files/figure-latex/cus-ac-plot-leg-cls-1.pdf}
...
## [16] flashClust_1.01-2    labeling_0.4.2       bookdown_0.24       
## [19] mosaicCore_0.9.0     scales_1.1.1         ggridges_0.5.3      
## [22] stringr_1.4.0        digest_0.6.28        ggformula_0.10.1    
## [25] rmarkdown_2.11       pkgconfig_2.0.3      htmltools_0.5.2     
## [28] geeM_0.10.1          labelled_2.9.0       fastmap_1.1.0       
## [31] highr_0.9            htmlwidgets_1.5.4    rlang_0.4.12        
## [34] readxl_1.3.1         rstudioapi_0.13      farver_2.1.0        
## [37] generics_0.1.1       car_3.0-12           dplyr_1.0.7         
## [40] plantecophys2_0.1.0  magrittr_2.0.1       leaps_3.1           
## [43] Matrix_1.3-4         Rcpp_1.0.7           munsell_0.5.0       
## [46] fansi_0.5.0          abind_1.4-5          geepack_1.3-2       
## [49] lifecycle_1.0.1      scatterplot3d_0.3-41 stringi_1.7.5       
## [52] yaml_2.2.1           carData_3.0-4        MASS_7.3-54         
## [55] ggstance_0.3.5       grid_4.1.1           ggrepel_0.9.1       
## [58] forcats_0.5.1        crayon_1.4.2         lattice_0.20-45     
## [61] haven_2.4.3          splines_4.1.1        hms_1.1.1           
## [64] knitr_1.36           pillar_1.6.4         ggpubr_0.4.0        
## [67] ggsignif_0.6.3       glue_1.4.2           evaluate_0.14       
## [70] png_0.1-7            vctrs_0.3.8          tweenr_1.0.2        
## [73] cellranger_1.1.0     gtable_0.3.0         polyclip_1.10-0     
## [76] tidyr_1.1.4          assertthat_0.2.1     xfun_0.27           
## [79] ggforce_0.3.3        broom_0.7.10         rstatix_0.7.0       
## [82] tibble_3.1.5         cluster_2.1.2        ellipsis_0.3.2
\end{verbatim}
