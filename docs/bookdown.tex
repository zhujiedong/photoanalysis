...
其中就两个参数，第一个为 LI-6800 \textbf{原始数据}所在的文件夹，即文件夹内\textbf{包含且仅包含 LI-6800 的原始数据}，不要放其他文件，不要放 excel 数据。第二个参数即为你原始数据中表头所在的参数行的\textbf{前一行}，我使用的这些示例数据的表头在 54 行。
...
\caption{\label{fig:unnamed-chunk-15}C4 植物 A-Ci 作图}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-32}jiptest 批量导入数据后的样式}
\centering
\begin{tabular}{rrl}
\toprule
SECS & FLUOR & SOURCE\\
...
\caption{\label{tab:unnamed-chunk-33}jiptest DC 数据批量导入数据后的样式}
\centering
\begin{tabular}{rrl}
\toprule
SECS & FLUOR & SOURCE\\
